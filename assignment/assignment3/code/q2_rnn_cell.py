...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable("W_x", [self.input_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            W_h = tf.get_variable("W_h", [self._state_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable("b", [self._state_size], initializer=tf.constant_initializer(0))

            z = tf.matmul(inputs, W_x)
            z = tf.add(z, tf.matmul(state, W_h))
            z = tf.add(z, b)
