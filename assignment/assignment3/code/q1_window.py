...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for t in xrange(len(sentence)):
            feature = []
            for i in xrange(t-window_size, t+window_size+1):
                if i < 0:
                    feature.extend(start)
                elif i >= len(sentence):
                    feature.extend(end)
                else:
                    feature.extend(sentence[i])
            windowed_data.append((feature, labels[t]))
        ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        input_emb = tf.nn.embedding_lookup(emb, self.input_placeholder)
        embeddings = tf.reshape(input_emb, shape=(-1, window_features*embedding_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        embedding_size = self.config.embed_size
        window_features = self.config.n_window_features
        hid_size = self.config.hidden_size
        class_size = self.config.n_classes

        W = tf.get_variable('W', [window_features*embedding_size, hid_size], initializer=tf.contrib.layers.xavier_initializer(seed=1))
        b1 = tf.get_variable('b1', [hid_size], initializer=tf.contrib.layers.xavier_initializer(seed=2))

        U = tf.get_variable('U', [hid_size, class_size], initializer=tf.contrib.layers.xavier_initializer(seed=3))
        b2 = tf.get_variable('b2', [class_size], initializer=tf.contrib.layers.xavier_initializer(seed=4))
    ...
    def add_loss_op(self, pred):
        ...
        ce = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
