...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        lens = len(sentence)
        diff = max_length - lens
        if diff > 0:
            new_sentence = sentence + ([zero_vector] * diff)
            new_labels = labels + ([zero_label] * diff)
            masking = [True] * lens + [False] * diff
        else:
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(input_emb, shape=(-1, max_length, n_features*embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        sp = tf.shape(x)
        state = tf.zeros([sp[0], self.config.hidden_size])
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, _ = cell(x[:, time_step, :], state, scope="RNN")
                o_drop_t = tf.nn.dropout(o_t, dropout_rate)
                y_t = tf.add(tf.matmul(o_drop_t, U), b2)
                preds.append(y_t)           
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        ce = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=masked_labels, logits=masked_preds)
