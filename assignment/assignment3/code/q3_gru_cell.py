...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_r = tf.get_variable("U_r", [self._state_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            W_r = tf.get_variable("W_r", [self.input_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            b_r = tf.get_variable("b_r", [self._state_size], initializer=tf.constant_initializer(0))

            U_z = tf.get_variable("U_z", [self._state_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            W_z = tf.get_variable("W_z", [self.input_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            b_z = tf.get_variable("b_z", [self._state_size], initializer=tf.constant_initializer(0))

            U_o = tf.get_variable("U_o", [self._state_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            W_o = tf.get_variable("W_o", [self.input_size, self._state_size], initializer=tf.contrib.layers.xavier_initializer())
            b_o = tf.get_variable("b_o", [self._state_size], initializer=tf.constant_initializer(0))

            # update gate
            z_t = tf.matmul(inputs, W_z) + tf.matmul(state, U_z) + b_z
            z_t = tf.sigmoid(z_t)

            # reset gate
            r_t = tf.matmul(inputs, W_r) + tf.matmul(state, U_r) + b_r
            r_t = tf.sigmoid(r_t)

            # new memory
            o_t = tf.matmul(inputs, W_o) + tf.matmul(r_t * state, U_o) + b_o
            o_t = tf.tanh(o_t)
