...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        pre_embeddings = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(pre_embeddings, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("transformation"):
            n_hidden_features = self.config.n_features * self.config.embed_size
            W = xavier([n_hidden_features, self.config.hidden_size])
            ...
            if self.config.extension_on:
                tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(self.config.lambda_rate)(W))
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=self.labels_placeholder)
...
def main(debug=True):
    ...
    with tf.Session(graph=graph) as session:
        ...
        if not debug:
            ...
            #print 'weight: {:}'.format(weight.eval())
